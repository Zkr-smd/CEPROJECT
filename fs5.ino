...
#include "CNN.h" // Include the CNN library
...
CNN cnn(NUM_CLASSES, NUM_EPOCHS, BATCH_SIZE, LEARNING_RATE);

//DATASET
#define NUM_SAMPLES 100
#define NUM_TEST_SAMPLES 10
float trainingData[NUM_SAMPLES][MFCC_SIZE];
float testData[NUM_TEST_SAMPLES][MFCC_SIZE];
int trainingLabels[NUM_SAMPLES];
int testLabels[NUM_TEST_SAMPLES];
...
void loop() {
  ...
  if (ADC->ADC_ISR & ADC_ISR_ENDRX) {
    ...
    // Test the CNN
    float mse = cnn.test(testData, testLabels);
    Serial.println("MSE: " + String(mse));
